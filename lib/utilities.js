...
module.exports = {
  ...
  async openFile(pathToFile) {
    ...
    await module.exports.runCmd(cmd);
  },
  ...
  renderDotGraph() {
    if (atom.packages.getLoadedPackages("graphviz-preview-plus")) {
      atom.commands.dispatch(atom.views.getView(atom.workspace.getActivePaneItem()), 'graphviz-preview-plus:toggle');
    }else{
      atom.notifications.addInfo("Install the GraphViz preview plus package to visualize the dot graph");
    }
  }
}
